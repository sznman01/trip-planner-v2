...
async function fetchLiveRate(from: CurrencyCode, to: CurrencyCode, signal?: AbortSignal) {
  ...
  // 兼容你 prep.txt 版本：json.ok + json.data.rates[to] [file:18]
  const rate = Number(json?.data?.rates?.[to]);
  if (!json?.ok || !Number.isFinite(rate) || rate <= 0) throw new Error("fx_bad_rate");
