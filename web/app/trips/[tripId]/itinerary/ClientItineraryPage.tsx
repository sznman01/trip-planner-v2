...
export default function ClientItineraryPagPage() {
  ...
  if (editingActivityFullIndex !== null) {
    // === 編輯模式 ===
    const index = editingActivityFullIndex
    const original = next.itinerary[index]
    
    if (original) {
      // ✅ 保留原有 id，只更新內容
      next.itinerary[index] = {
        ...original,  // 保留 id 和其他原有屬性
        ...payload,   // 覆蓋新內容
      }
    }
  } else {
    ...
    // ✅ 必須產生唯一 id
    next.itinerary.push({
      id: crypto.randomUUID(), // 唯一 ID (瀏覽器內建)
      ...payload,
    })
  }
})
            ...
            {currentDayActivities.map(({ item, fullIndex }, idx) => {
              ...
              return (
                <div key={`${fullIndex}-${item.time}-${item.name}`} className="relative">
                  {!isLast ? (
                    <div className="absolute left-5 top-12 h-10 w-1 rounded-full bg-gradient-to-b from-rose-400 to-rose-600" />
                  ) : null}
          ...
          <div className="w-full max-w-md overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-2xl">
            <div className="p-6">
              ...
              <div className="mt-5 space-y-3">
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700">地點</label>
                  <input
                    value={placeDraft}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-2xl">
            <div className="p-6">
              ...
              <div className="mt-5 space-y-3">
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700">時間</label>
                  <input
                    type="time"
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700">名稱</label>
                  <input
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700">類型</label>
                  <select
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700">備註</label>
                  <textarea
